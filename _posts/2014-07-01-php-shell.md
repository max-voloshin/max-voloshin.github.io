...
excerpt: "PHP разработчики в большинстве случаев занимаются разработкой веб-приложений, где отправной точкой запуска исходного кода является запрос к веб-серверу.
          Несмотря на это, время от времени мы работаем с PHP из Shell, и для того чтобы делать это более эффективно стоит разобраться в деталях пересечения PHP и Shell.
"
